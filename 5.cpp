...
void draw_one(int num,int i,int j,double a,double b,double c,double d){
    double x1,x2,y1,y2,dx,dy,ox,oy;
...
void draw_opposite(int num,int i,int j,double a,double b,double c,double d){
    double x1,x2,x3,x4,y1,y2,y3,y4,dx,dy,ox,oy;
    dx=(X_MAX-X_MIN)/(N_X-1.0);
    dy=(Y_MAX-Y_MIN)/(N_Y-1.0);
    ox=X_MIN+i*dx;
    oy=Y_MIN+j*dy;
...
void myReshape(int w,int h){
    ...
    if(w<=h){
        ...
        gluOrtho2D(X_MIN, X_MAX, Y_MIN*ar, Y_MAX*ar);
    }else{
        GLfloat ar1=(GLfloat)w/(GLfloat)h;
        gluOrtho2D(X_MIN*ar1, X_MAX*ar1, Y_MIN, Y_MAX);
    }
